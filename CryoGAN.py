...
class CryoGAN:
    ...
    def optimize_discriminator(self, real_samples):
        ...
        if self.args.dis_clip_grad == True:
            current_clip_val = 1e3 + (self.args.dis_clip_norm_value - 1e3) * 0.5 * self.step / float(
                self.epoch_gen_iterations)
    ...
    def optimize_generator(self, real_samples, multipleNoise):
        ...
        if self.args.gen_clip_grad == True:
            current_clip_val = 1 + (self.args.gen_clip_norm_value - 1) * 0.5 * self.step / float(
                self.epoch_gen_iterations)
