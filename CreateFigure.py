...
class CreateFigure:
    ...
    def __init__(self, depth=7, latent_size=32, learning_rate=0.001, beta_1=0.5,
                 ...
                 device=torch.device("cpu"), args=None):
    ...
    def ReconstructedSequence(self, OUTPUT_PATH):
        ...
        lists = [os.path.join(OUTPUT_PATH, fileName) for fileName in os.listdir(OUTPUT_PATH) if
                 ('.mrc' in fileName) and ('volume' not in fileName)]
    ...
    def registerVolumes(self, GroundTruth, Reconstruction, VolumeMask, FigurePath, StableVolume='GroundTruth'):
        ...
        os.system(
            "gcc  main.c  BsplnTrf.c  convolve.c     pyrFilt.c   quant.c reg1.c reg3.c  svdcmp.c BsplnWgt.c  getPut.c    message.c  pyrGetSz.c  reg0.c   reg2.c  regFlt3d.c -lm -o main")
        os.system("./main")
    ...
    def fsc(self, x1, x2, VolumeMask, size, resolution=0.637):
        ...
        with torch.no_grad():
            ...
            for i in range(len(freqInt) - 1):
                ...
                print(
                    "Correlation at " + str(resolution / (freqInt[i].cpu().item() + 1e-12)) + ' is ' + str(y[i].item()))
    ...
    def Create(self, epochs):
        ...
        plt.plot(freq.numpy(), fscBefore.numpy(), '-k', freq.numpy(), fscAfter.numpy(), '-b', freq.numpy(),
                 0.5 * np.ones((len(freq.numpy()), 1)), '--k')

        fig.savefig(os.path.join(FigurePath, 'FSC.png'))

        # plot Projections
        self.gen.X.data = GroundTruth
        real_samples, realNoisy, realCTF, realClean = self.gen(noise=None, depth=None, alpha=None,
                                                               skipDownsampling=True, ChangeAngles=False)

        self.gen.X.data = ReconstructionRegistered
        fake_samples, fakeNoisy, fakeCTF, fakeClean = self.gen(noise=None, depth=None, alpha=None,
                                                               skipDownsampling=True, ChangeAngles=False)
        ...
        save_fig_single_separate(realClean.cpu().data, FigurePath, name, nrow=1, iteration=None, doCurrent=False,
                                 figshow=False, Title=None, scaleEach=False)

        name = 'Fake Clean Data'
        save_fig_single_separate(fakeClean.cpu().data, FigurePath, name, nrow=1, iteration=None, doCurrent=False,
                                 figshow=False, Title=None, scaleEach=False)

        name = 'Real CTF Data'
        save_fig_single_separate(realCTF.cpu().data, FigurePath, name, nrow=1, iteration=None, doCurrent=False,
                                 figshow=False, Title=None, scaleEach=False)

        name = 'Fake CTF Data'
        save_fig_single_separate(fakeCTF.cpu().data, FigurePath, name, nrow=1, iteration=None, doCurrent=False,
                                 figshow=False, Title=None, scaleEach=False)
        ...
        for i, volume in enumerate(ReconstructedSequence):
            ...
            with mrcfile.new(os.path.join(FigurePath, 'registered' + UnstableVolume + baseName + '.mrc'),
                             overwrite=True) as m:
                m.set_data(ReconstructionRotated)

            self.gen.X.data = torch.Tensor(ReconstructionRotated).to(self.device)

            proj_samples, projNoisy, projCTF, projClean = self.gen(noise=None, depth=None, alpha=None,
                                                                   skipDownsampling=True, ChangeAngles=False)
            ...
            save_fig_single_separate(projClean.cpu().data, FigurePath, name, nrow=3, iteration=None, doCurrent=False,
                                     figshow=False, Title=None, scaleEach=False)
