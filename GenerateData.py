...
class GenerateData:
    ...
    def initGT(self, device):
        ...
        if 'Ribo' in self.args.dataset:
            ...
            fittedBetagal = torch.Tensor(mrcfile.open('./Datasets/Betagal-Synthetic/fitted_betagal_2.5A.mrc').data).to(
                device)
            fittedBetagal = torch.nn.functional.avg_pool3d(fittedBetagal.unsqueeze(0).unsqueeze(0),
                                                           kernel_size=self.args.DownSampleRate,
                                                           stride=self.args.DownSampleRate, padding=0).squeeze()
        ...
        if self.args.VolumeDomain == 'fourier' and self.args.FourierProjector == True:
            self.gen.X.data = fftshift(
                torch.rfft(ifftshift(self.GroundTruth, mode='real', signal_dim=3), 3, onesided=False), mode='complex',
                signal_dim=3)
        ...
        for i in range(epochs // self.args.BATCH_SIZE):
            ...
            with torch.no_grad():
                ...
                for j, projNoisySingle in enumerate(projNoisy):
                    ...
                    with mrcfile.new(os.path.join(sample_dir, 'projNoisy' + str(num).zfill(6) + '.mrc'),
                                     overwrite=True) as m:
                ...
                fscNoise, fscCTF, fscClean, fscRatio = SaveRadial(Noise, projCTF, projClean, OUTPUT_PATH,
                                                                  fscNoise=fscNoise, fscCTF=fscCTF, fscClean=fscClean,
                                                                  fscRatio=fscRatio, save=save)
                ...
                if save:
                    ...
                    if self.args.dataset == 'bs':
                        for numvol, vol in enumerate(self.gen.X.data):
                            with mrcfile.new(
                                    os.path.join(sample_dir, 'GroundTruth_' + str(num) + '_' + str(i) + '.mrc'),
                                    overwrite=True) as m:
...
def SaveRadial(Noise, projCTF, projClean, OUTPUT_PATH, fscNoise=None, fscCTF=None, fscClean=None, fscRatio=None,
               save=False):
