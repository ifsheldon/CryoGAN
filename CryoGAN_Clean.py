...
class ProGAN:
        ...
        if self.args.AlgoType != "generate":
            ...
            if self.args.UseOtherDis:
                try:
                    ...
                    listdir = ['./Results/' + self.args.dataset + '/' + dirname for dirname in listdir if
                               self.args.name[:-5] in dirname and self.args.data_type not in dirname]
                    if len(listdir) > 0:
        ...
        for epoch in range(1, self.args.epochs + 1):
            ...
            for i in range(epoch_gen_iterations):
                if self.args.GaussianFilterProjection:
                    ...
                    self.args.GaussianSigma = (perc * self.args.GaussianSigmaEpochFinal + (
                            1.0 - perc) * self.args.GaussianSigmaEpochInitial)
                ...
                gen_loss, fake_samples, projNoisy, projCTF, projClean, Noise = self.optimize_generator(
                    ChangeAngles=True)
                ...
                if (i % feedback_factor) == 0:
                    ...
                    print("Scalar1: [%f] Scalar2: [%f] " % (
                        torch.exp(self.gen.G.scalar[0]).item(), torch.exp(self.gen.G.scalar[1]).item()))
    ...
    def optimize_discriminator(self, real_samples):
        ...
        loss, wass_loss = dis_loss(self.dis, real_samples, fake_samples, self.args.lambdaPenalty,
                                   self.args.lambda_drift, self.args.gamma_gradient_penalty)

        if self.args.dis_clip_grad == True:
            clip_norm_value = np.min(
                [1e3 + (self.args.dis_clip_norm_value - 1e3) * 0.5 * self.step / float(self.epoch_gen_iterations),
                 self.args.dis_clip_norm_value])
    ...
    def optimize_generator(self, real_samples, multipleNoise):
        ...
        fake_samples, projNoisy, projCTF, projClean, Noise = self.gen(real_samples,
                                                                      GaussianSigma=self.args.GaussianSigma,
                                                                      ratio=self.ratio, multipleNoise=multipleNoise)
        ...
        if self.args.gen_clip_grad == True:
            clip_norm_value = np.min(
                [1 + (self.args.gen_clip_norm_value - 1) * 0.5 * self.step / float(self.epoch_gen_iterations),
                 self.args.gen_clip_norm_value])
