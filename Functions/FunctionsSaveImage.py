...
def save_fig(x, path, name, iteration=None, doCurrent=False, figshow=False, Title=None, scaleEach=False):
    ...
    im = np.transpose(
        make_grid(x, nrow=int(np.sqrt(x.size()[0])), padding=0, scale_each=scaleEach, normalize=scaleEach).numpy(),
        (1, 2, 0))[:, :, 0]
...
def save_fig_single(x, path, name, nrow=None, iteration=None, doCurrent=False, figshow=False, Title=None,
                    scaleEach=False):
    ...
    im = np.transpose(
        make_grid(x, nrow, padding=int(0.05 * x.shape[-1]), scale_each=scaleEach, normalize=scaleEach).numpy(),
        (1, 2, 0))[:, :, 0]
    plt.imshow(im, **imshowParams)
    plt.axis('off')
...
def save_fig_single_separate(x, path, name, nrow=None, iteration=None, doCurrent=False, figshow=False, Title=None,
                             scaleEach=False, vminvalue=None, vmaxvalue=None):
    Title = name if Title is None else Title
    nrow = int(np.sqrt(x.size()[0])) if nrow is None else nrow
