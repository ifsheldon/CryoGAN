...
def SpaceToFourier(X, signal_dim=3, normalized=True):
    XF = fftshift(
        torch.rfft(ifftshift(X, mode='real', signal_dim=signal_dim), signal_dim, onesided=False, normalized=normalized),
        mode='complex', signal_dim=signal_dim)
    return XF


def FourierToSpace(XF, signal_dim=3, normalized=True):
    X = fftshift(torch.irfft(ifftshift(XF, mode='complex', signal_dim=signal_dim), signal_dim, onesided=False,
                             normalized=normalized), mode='real', signal_dim=signal_dim)
    return X
...
def FFTConv(imgs, filt, plot=False):
    ...
    if plot == True:
        save_fig_double(filtR.data.cpu(), filtIm.data.cpu(), './', 'CurrentCTF-Fourier', iteration=None, Title1='Real',
                        Title2='Imag')
        save_fig_double((imgR + 1e-8).abs().log().data.cpu(), (imgIm + 1e-8).abs().log().data.cpu(), './',
                        'CurrentProj-Fourier', iteration=None, Title1='Real', Title2='Imag')
