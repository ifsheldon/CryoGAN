...
def CTFGenerator(args, defocusU, defocusV, AngleAstigmatism):
    ...
    defocusU = defocusU.view(-1, 1, 1, 1) + defocusU.view(-1, 1, 1, 1) * 0.0 * torch.zeros_like(
        defocusU.view(-1, 1, 1, 1)).normal_()

    defocusV = defocusV.view(-1, 1, 1, 1) + defocusV.view(-1, 1, 1, 1) * 0.0 * torch.zeros_like(
        defocusV.view(-1, 1, 1, 1)).normal_()
