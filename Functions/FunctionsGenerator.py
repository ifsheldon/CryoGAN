...
def CTFforward(h, x):
    ...
    xConvolved = FFTConv(xpadded, h, plot=doPlots)[0][:, :, paddingLength: paddingLength + x.size()[-2],
                 paddingLength:paddingLength + x.size()[-1]]
...
def InitVolume(args):
    ...
    if args.SymmetryType == 'none':
        ...
        if args.SymmetryType == 'D' and n == 2:
            ...
            Xd = Tensor(args.VolumeNumbers, (args.VolumeSize) // 2, (args.VolumeSize) // 2, (args.VolumeSize)).fill_(
                0).cuda()
            ...
            Xd = Tensor(args.VolumeNumbers, (args.VolumeSize // args.d), (args.VolumeSize // args.d),
                        2 * (args.VolumeSize // args.d)).fill_(0).cuda()
...
def InitMask(args, Xd=None, X=None, radius=None):
    ...
    if args.SymmetryType == "D":
        ...
        xx, yy, zz = torch.meshgrid(torch.arange(0, Xd.size()[1]).cuda(), torch.arange(0, Xd.size()[2]).cuda(),
                                    torch.arange(0, Xd.size()[3]).cuda())
        ...
        maskd = torch.zeros(Xd.shape[-3:]).cuda()
        xx, yy, zz = torch.meshgrid(torch.arange(0, Xd.size()[1]), torch.arange(0, Xd.size()[2]),
                                    torch.arange(0, Xd.size()[3]))

        xx = xx.contiguous().view(-1)
        yy = yy.contiguous().view(-1)
        zz = zz.contiguous().view(-1)
        valueMask = (((xx - maskd.shape[0]).pow(2) + (yy - maskd.shape[1]).pow(2)).float().sqrt() < np.max(
            [args.VolumeMaskSize, 0.5]) * maskd.shape[0]) * (
                            (zz - maskd.shape[2] // 2).float().abs() < args.VolumeMaskSize * maskd.shape[2] // 2)

        radius = (args.VolumeSize // 2) * radius
        valueMask = (((xx - maskd.shape[0]).pow(2) + (yy - maskd.shape[1]).pow(2) + (zz - maskd.shape[2] // 2).pow(
            2)).float().sqrt() < radius)
        ...
        yy = yy.contiguous().view(-1)
        zz = zz.contiguous().view(-1)
        valueMask = (((xx - maskd.shape[0]).pow(2) + (yy - maskd.shape[1] // 2).pow(2)).float().sqrt() < np.max(
            [args.VolumeMaskSize, 0.5]) * maskd.shape[0]) * (
                            (zz - maskd.shape[2] // 2).float().abs() < args.VolumeMaskSize * maskd.shape[2] // 2)

        radius = (args.VolumeSize // 2) * radius
        valueMask = (((xx - maskd.shape[0]).pow(2) + (yy - maskd.shape[1] // 2).pow(2) + (zz - maskd.shape[2] // 2).pow(
            2)).float().sqrt() < radius)
        ...
        xx, yy, zz = torch.meshgrid(torch.arange(0, X.size()[1]), torch.arange(0, X.size()[2]),
                                    torch.arange(0, X.size()[3]))
    ...
    def backward(ctx, grad_out):
        ...
        if ctx.needs_input_grad[0]:
            ...
            bproj_id, bproj_data = astra.create_backprojection3d_gpu(
                grad_out.squeeze(1).permute(2, 0, 1).data.cpu().numpy(), ctx.proj_geom, ctx.vol_geom)
