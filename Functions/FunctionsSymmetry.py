...
def SymmetryFunction(xd, N=64, d=7, mode='d'):
    ...
    if mode == 'd':
        ...
        xpol = (xdFull.unsqueeze(1) + torch.zeros(xdFull.size()[0], d, xdFull.size()[1], xdFull.size()[2]).cuda()).view(
            xdFull.size()[0], -1, xdFull.size()[2])
...
def PolToCartGrid(n, device):
    ...
    rr = torch.sqrt(
        2 * (xx.pow(2) + yy.pow(2)))  # -1+ (2**0.5)*2*((xx.float()/n -0.5).pow(2)+(yy.float()/n -0.5).pow(2)).sqrt()
    tt = (torch.atan2(yy, xx) + np.pi) * n / (
                2 * np.pi)  # -1+2*((yy>=n//2).float()*np.pi/2.0+torch.atan((xx.float()-n//2).float()/(yy.float()-n//2 + 1e-12).float())/2.0+np.pi/4.0)/np.pi
