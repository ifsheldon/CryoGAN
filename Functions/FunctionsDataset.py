...
def CoordsLOG(path, micrographNum, threshold=False, N=None, sizeParticle=384):
    ...
    if N is not None:
        ...
        return coordsLOG[
            (coordsLOG[:, 2] > threshold).nonzero()[0]]  # return particles with prob greater than the threshold
    else:
        return coordsLOG
...
def CoordsBackground(pathCoords, pathMicrograph, threshold=False, NumberParticles=10, sizeParticle=384):
    ...
    for p in coordsLOG:
        ...
        isOK[np.amax(y - M // 2, initial=0):  np.amin(y - M // 2 + M, initial=isOK.shape[0] - 1),
        np.amax(x - M // 2, initial=0):  np.amin(x - M // 2 + M,
                                                 initial=isOK.shape[1] - 1)] = False  # intentional swap, y first!
...
def CoordsBackground_Betagal_Std(pathCoords, pathMicrograph, threshold=False, NumberParticles=10, sizeParticle=384,
                                 downSample=1):
    N = sizeParticle
    M = 2 * N
    ...
    for p in range(NumberParticles):
        ...
        heatmap[ind[0] - sizeParticle // 4: ind[0] + sizeParticle // 4,
        ind[1] - sizeParticle // 4: ind[1] + sizeParticle // 4] = val
    return coords
...
def ConvolveTemplate(micrograph, template, downSample=1):
    ...
    convolved = torch.nn.functional.conv2d(Down(micrograph, downSample=1), Down(template, downSample=1), padding=(
    template.shape[-2] // (2 * downSample), template.shape[-1] // (2 * downSample)))
