...
class CreateFigure:
    ...
    def Create(self, epochs):
        ...
        for numFigure in range(1):
            ...
            save_fig_single_separate(real_samples.cpu().data, FigurePath, name, nrow=1, iteration=None, doCurrent=False,
            ...
            save_fig_single_separate(fake_samples.cpu().data, FigurePath, name, nrow=1, iteration=None, doCurrent=False,
                                     figshow=False, Title=None, scaleEach=False)

            name = 'Fake Clean Data Even ' + str(numFigure + 1)
            save_fig_single_separate(projClean.cpu().data, FigurePath, name, nrow=1, iteration=None, doCurrent=False,
                                     figshow=False, Title=None, scaleEach=False)

            name = 'Fake CTF Data Even ' + str(numFigure + 1)
            save_fig_single_separate(projCTF.cpu().data, FigurePath, name, nrow=1, iteration=None, doCurrent=False,
                                     figshow=False, Title=None, scaleEach=False)
            ...
            for i, volume in enumerate(ReconstructedSequenceEven):
                ...
                save_fig_single_separate(projClean.cpu().data, FigurePathCurrent, name, nrow=1, iteration=None,
                                         doCurrent=False, figshow=False, Title=None, scaleEach=False)

                name = baseName + 'Fake Data' + str(numFigure + 1)
                save_fig_single_separate(proj_samples.cpu().data, FigurePathCurrent, name, nrow=1, iteration=None,
                                         doCurrent=False, figshow=False, Title=None, scaleEach=False)

                name = baseName + 'Fake CTF Data' + str(numFigure + 1)
                save_fig_single_separate(projCTF.cpu().data, FigurePathCurrent, name, nrow=1, iteration=None,
                                         doCurrent=False, figshow=False, Title=None, scaleEach=False)
    ...
    def ReconstructedSequence(self, OUTPUT_PATH):
        ...
        lists = [os.path.join(OUTPUT_PATH, fileName) for fileName in os.listdir(OUTPUT_PATH) if
                 ('.mrc' in fileName) and ('reconstruction' in fileName)]
    ...
    def Figure(self, fsc, freq, fsc_names, FSC_numbers, figure_name, ext=".png", TimePer=40):
        ...
        fsc_names = [str(TimePer * (i + 1)) + " mins: " + ("% 2.2f" % FSC_number).zfill(4) + " A" for
