...
class cryoGenerator(th.nn.Module):
    ...
    def __init__(self, args=None):
        ...
        self.X = th.nn.Parameter(self.G.X.clone().detach()) if self.G.X is not None else th.nn.Parameter(
            self.G.Xd.clone().detach())
    ...
    def forward(self, real_samples, skipDownsampling=False, ChangeAngles=True, GaussianSigma=None, ratio=0,
                multipleNoise=False):
        self.iteration = self.iteration + 1
        """
        forward pass of the Generator
        """
        projNoisy, projCTF, projClean, self.Volume, Noise = self.G(self.X, real_samples, ChangeAngles=ChangeAngles,
                                                                   ratio=ratio)
        ...
        if self.args.UseVolumeGenerator == False:
            with torch.no_grad():
                ...
                if self.args.VolumeMask == False:
                    _, _, self.xx, self.yy, self.zz, self.centrex, self.centrey, self.centrez, self.radius = InitMask(
                        self.args, self.G.Xd, self.G.X, self.VolumeMaskRadius)
                if self.args.Constraint == 'Positive':
                    ...
                    radial = ((self.xx - self.centrex) ** 2 + (self.yy - self.centrey) ** 2 + (
                            self.zz - self.centrez) ** 2).float().sqrt().view(self.X.data.shape[1],
                                                                              self.X.data.shape[2],
                                                                              self.X.data.shape[3])
        ...
        with torch.no_grad():
            if self.args.VolumeMask:
                ...
                if self.VolumeMaskRadiusNew != self.VolumeMaskRadiusPrev:
                    self.mask, self.maskd, self.xx, self.yy, self.zz, self.centrex, self.centrey, self.centrez, self.radius = InitMask(
                        self.args, self.G.Xd, self.G.X, self.VolumeMaskRadiusNew)
