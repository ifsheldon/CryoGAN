...
dataDir = "./Datasets/betagal/Micrographs"
...
for micrographNum in range(0, 1539):
    ...
    coordsBackground = CoordsBackground_Betagal_Std(imSmoothDS, threshold=False, NumberParticles=len(boxes),
                                                    sizeParticle=N, downSample=1)
    ...
    for i in range(len(coordsBackground)):
        ...
        with mrcfile.new(os.path.join(pathSaveBackground, str(counterBackground).zfill(6) + ".mrc"),
                         overwrite=True) as m:
            m.set_data(imBack)
