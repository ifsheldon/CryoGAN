...
class VarphiGenerator(Dataset):
    ...
    def TranslationGenerator(self, idx):
        ...
        if self.args.UseEstimatedTranslations == True:
            ...
            Translation = (
                        U2T(TranlsationNormalized) * self.args.RawProjectionSize * self.args.TranslationVariance / 100.0).float().long()  # should be even
        return Translation
...
def EstmatedNoise(args):
    try:
        ...
        args.EstimatedNoiseIterator = iter(
            torch.utils.data.DataLoader(args.EstimatedNoiseDataset, batch_size=args.batch_size, shuffle=True,
