...
class CreateFigure:
    ...
    def FSC(self, Path1, Path2, OutPath, LP=True, Average=True):
        ...
        command = "focus.postprocess " + Path1 + " " + Path2 + " --angpix " + str(
            resolution) + " --automask --auto_bfac -1,-1 --out " + OutPath
        print(command)
        os.system(command)
        ...
        if LP:
            print("Low Pass")
            command = "focus.postprocess " + Path1 + " " + Path1 + " --angpix " + str(
                resolution) + " --automask --auto_bfac -1,-1 " + "--lowpass " + ("% 2.2f" % FSC_masked_recon).zfill(
                4) + " --out " + OutPath + "_LP_Even"

            os.system(command)
            if not os.path.exists(OutPath + "_LP_Even" + "_fsc-masked.txt"):
                time.sleep(5)

            command = "focus.postprocess " + Path2 + " " + Path2 + " --angpix " + str(
                resolution) + " --automask --auto_bfac -1,-1 " + "--lowpass " + ("% 2.2f" % FSC_masked_recon).zfill(
                4) + " --out " + OutPath + "_LP_Odd"
            ...
            if Average:
                ...
                command = "focus.postprocess " + OutPath + "_Average.mrc" + " " + OutPath + "_Average.mrc" + " --angpix " + str(
                    resolution) + " --automask --auto_bfac -1,-1 " + "--lowpass " + ("% 2.2f" % FSC_masked_recon).zfill(
                    4) + " --out " + OutPath + "_LP_Average"
                os.system(command)
    ...
    def Create(self, epochs):
        ...
        if Plot == True:
            ...
            for i, real_samples_single in enumerate(real_samples):
                ...
                save_fig_single_separate(real_samples_single.cpu().data, FigurePath, name, nrow=1, iteration=None,
                                         doCurrent=False, figshow=False, Title=None, scaleEach=False, vminvalue=-4,
                                         vmaxvalue=4.75)
            ...
            self.PlotImages(fake_samples, projCTF, projClean, FigurePath, Prefix="Fake_Even", Suffix="",
                            numFigureCounter=1)
            ...
            for i, volume in enumerate(ReconstructedSequenceEven):
                ...
                self.PlotImages(proj_samples, projCTF, projClean, FigurePathCurrent, Prefix=baseName, Suffix="",
                                numFigureCounter=1)
            ...
            self.PlotImages(fake_samples, projCTF, projClean, FigurePath, Prefix="Fake_Odd", Suffix="",
                            numFigureCounter=1)
        ...
        for i in range(len(ReconstructedSequenceEven)):
            ...
            fsc_masked_single, fsc_unmasked_single, freq = self.FSC(FigurePathCurrentEven + baseName + "_Even.mrc",
                                                                    FigurePathCurrentOdd + baseName + "_Odd.mrc",
                                                                    os.path.join(FigurePathCurrent, "FSC_" + baseName),
                                                                    LP=True, Average=True)
        ...
        FSC_number_masked_firsthalf = [FSC_number_masked_s for i, FSC_number_masked_s in enumerate(FSC_number_masked) if
                                       i in vec]
        FSC_number_unmasked_firsthalf = [FSC_number_unmasked_s for i, FSC_number_unmasked_s in
                                         enumerate(FSC_number_unmasked) if i in vec]
        ...
        self.Figure(fsc_unmasked_firsthalf, freq, fsc_names_firsthalf, FSC_number_unmasked_firsthalf,
                    os.path.join(FigurePath, "FSC_Unmasked_OnlyTill40"), TimePer=100)
        ...
        self.Figure(fsc_unmasked, freq, fsc_names, FSC_number_unmasked,
                    os.path.join(FigurePath, "FSC_Unmasked_Alternate"), TimePer=200)
        ...
        if Plot_GT_FSC:
            for i in range(len(ReconstructedSequenceEven)):
                ...
                fsc_masked_even_single, fsc_unmasked_even_single, freq = self.FSC(
                    FigurePathCurrentEven + baseName + "_Even.mrc", GroundTruthPath,
                    os.path.join(FigurePathCurrentEven, "FSC_GT_Even_" + baseName))
            ...
            self.Figure(fsc_masked_even, freq, fsc_names_even, FSC_number_masked_even,
                        os.path.join(FigurePath, "FSC_Masked_GT_Even"))
            self.Figure(fsc_unmasked_even, freq, fsc_names_even, FSC_number_unmasked_even,
                        os.path.join(FigurePath, "FSC_Unmasked_GT_Even"))
            ...
            for i in range(len(ReconstructedSequenceOdd)):
                ...
                baseName = str(10 * (i)).zfill(2) + '_Iteration'
                fsc_masked_odd_single, fsc_unmasked_odd_single, freq = self.FSC(
                    FigurePathCurrentOdd + baseName + "_Odd.mrc", GroundTruthPath,
                    os.path.join(FigurePathCurrentOdd, "FSC_GT_Odd_" + baseName))
            ...
            self.Figure(fsc_masked_odd, freq, fsc_names_odd, FSC_number_masked_odd,
                        os.path.join(FigurePath, "FSC_Masked_GT_Odd"))
            self.Figure(fsc_unmasked_odd, freq, fsc_names_odd, FSC_number_unmasked_odd,
                        os.path.join(FigurePath, "FSC_Unmasked_GT_Odd"))
    ...
    def GroundTruthPath(self):
        ...
        OUTPUT_PATH = './Datasets/Betagal-Synthetic/molmap2.5-Downsampled' + str(
            self.args.DownSampleRate) + '-translationOff-EstimatedNoise-snr_ratio_0.4/GroundTruth_0.mrc'
    ...
    def EvenPath(self):
        ...
        lists = [OUTPUT_PATH + folderName for folderName in os.listdir(OUTPUT_PATH) if
                 name in folderName and "even" in folderName]
    ...
    def OddPath(self):
        ...
        name = self.args.name[:-4]
        lists = [OUTPUT_PATH + folderName for folderName in os.listdir(OUTPUT_PATH) if
                 name in folderName and "odd" in folderName]
    ...
    def ReconstructedSequence(self, OUTPUT_PATH):
        ...
        lists = [os.path.join(OUTPUT_PATH, fileName) for fileName in os.listdir(OUTPUT_PATH) if
                 ('.mrc' in fileName) and ('reconstruction' in fileName) and ('Epoch' not in fileName)]
    ...
    def SaveOrthoSlices(self, volume, FigurePath, name):
        ...
        save_fig_single_separate(slice_yz, FigurePath, name + '-yz', nrow=1, iteration=None, doCurrent=False,
                                 figshow=False, Title=None, scaleEach=False)
        save_fig_single_separate(slice_xz, FigurePath, name + '-xz', nrow=1, iteration=None, doCurrent=False,
                                 figshow=False, Title=None, scaleEach=False)
        save_fig_single_separate(slice_xy, FigurePath, name + '-xy', nrow=1, iteration=None, doCurrent=False,
                                 figshow=False, Title=None, scaleEach=False)
    ...
    def Figure(self, fsc, freq, fsc_names, FSC_numbers, figure_name, ext=".png", TimePer=20):
        ...
        fsc_names = [str(TimePer * (i)) + " mins: " + ("% 2.2f" % FSC_number).zfill(4) + " A" for
                     i, (fsc_name, FSC_number) in enumerate(zip(fsc_names, FSC_numbers))]
    ...
    def PlotImages(self, fake_samples, projCTF, projClean, FigurePath, Prefix="", Suffix="", numFigureCounter=1):
        ...
        for i, (fakesingle, projCTFsingle, projCleansingle) in enumerate(zip(fake_samples, projCTF, projClean)):
            ...
            save_fig_single_separate(fakesingle.cpu().data, FigurePath, name, nrow=1, iteration=None, doCurrent=False,
                                     figshow=False, Title=None, scaleEach=False, vminvalue=-4, vmaxvalue=5.25)

            name = Prefix + '_Clean_Data_' + Suffix + str(numFigureCounter + i)
            save_fig_single_separate(projCleansingle.cpu().data, FigurePath, name, nrow=1, iteration=None,
                                     doCurrent=False, figshow=False, Title=None, scaleEach=False, vminvalue=vminClean,
                                     vmaxvalue=vmaxClean)

            name = Prefix + '_CTF_Data_' + Suffix + str(numFigureCounter + i)
            save_fig_single_separate(projCTFsingle.cpu().data, FigurePath, name, nrow=1, iteration=None,
                                     doCurrent=False, figshow=False, Title=None, scaleEach=False, vminvalue=vminCTF,
                                     vmaxvalue=vmaxCTF)
