...
class ProjectorModule(nn.Module):
    ...
    def forward(self, X, ChangeAngles=True):

        X = self.Sym(X, self.S)
        X = torch.exp(self.scalar[0]) * X

        angles, translation, defocusU, defocusV, astigmatism, Noise = self.VarphiGeneratorBatch()
        if ChangeAngles == True or self.CurrentAngles is None: self.CurrentAngles = angles
    ...
    def NoiseAddition(self, projCTF, projClean, Noise, multipleNoise=False):
        ...
        if self.args.NormalizeProjections == True: projNoisy = self.B(projNoisy)
        if self.args.InvertProjections: projNoisy *= -1
    ...
    def VarphiGeneratorBatch(self):
        ...
        try:
            ...
            self.VarphiGeneratorLoader = iter(
                torch.utils.data.DataLoader(self.VarphiGenerator, batch_size=self.args.BATCH_SIZE, shuffle=True,
